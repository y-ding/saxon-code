...
class SAXON():
    def __init__(self, configs, threshold, nu=0.5, model_type = 'gp', gamma = 2.5):
        ...
        if model_type != 'gp':
            self.model_type = 'saxon_'+model_type
        elif gamma >= 2.25:
            self.model_type = 'saxon_nr'
        else:
            self.model_type = 'saxon_' + str(int(gamma*100))
...
def run_saxon(init_idx, benchmark, model, threshold, directory, sleep_time = 20):
    random.seed(0)
